...
# BUN Directory
if [ -d "$BUN_INSTALL/bin:$PATH" ]; then
    export PATH="$BUN_INSTALL/bin:$PATH"
fi

# Homebrew Directory
if [ -d "$(brew --prefix)/opt/python@3.11/libexec/bin:$PATH" ]; then
    export PATH="$(brew --prefix)/opt/python@3.11/libexec/bin:$PATH"
fi

# VMN Directory
if [ -d "$HOME/Personal/VMN:$PATH" ]; then
    export PATH="$HOME/Personal/VMN:$PATH"
fi

# Python User Script Directory for MacOS
if [ -d "$HOME/Library/Python/3.11/bin:$PATH" ]; then
